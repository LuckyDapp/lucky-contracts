...
pub mod psp22_reward {
    ...
    mod tests {
        ...
        fn test_fund_rewards_and_add_winners_insufficient_transferred_value() {
            ...
            // 600 > 100 => error
            let result = ink::env::pay_with_call!(
                contract.fund_rewards_and_add_winners(era, [(accounts.alice, 600)].to_vec()),
                100
            );

            match result {
                Err(InsufficientTransferredBalance) => {
                    debug_println!("Insufficient Transferred Balance as expected")
                }
                _ => panic!("Error 1!"),
            };
        ...
        fn test_fund_rewards_and_add_winners() {
            ...
            ink::env::pay_with_call!(
                contract.fund_rewards_and_add_winners(1, [(accounts.alice, 600)].to_vec()),
                1000
            )
            .unwrap();
            ...
            ink::env::pay_with_call!(
                contract.fund_rewards_and_add_winners(2, [(accounts.bob, 400)].to_vec()),
                1000
            )
            .unwrap();
            ...
            ink::env::pay_with_call!(
                contract.fund_rewards_and_add_winners(
                    3,
